								...
								<tbody id='tb'>
									<c:forEach var="article" items="${articles}">
										...
										<tr>
											...
											<td style="vertical-align: middle; flex:auto"><span>${article.memberPk}</span>
											<td style="vertical-align: middle; flex:auto"><span>${article.updateTime}</span>
											<td style="vertical-align: middle; flex:auto"><span>${article.topicName}</span>
											<td style="vertical-align: middle; flex:auto"><span>${article.title}</span>
											<td style="vertical-align: middle; flex:auto"><span>${article.content}</span>
											<td style="vertical-align: middle; flex:auto"><span>${article.status}</span>
											<td style="vertical-align: middle; flex:auto"><span>${article.like}</span>
											<td style="vertical-align: middle; flex:auto"><span>${article.dislike}</span>
											
											<td>
												<form  method="post" action="">
													<input type="hidden" name="articleId" value="${article.articleId}">
													<input type="submit" value="圖片資料庫" formaction="<c:url value='/imgDB_CMS' />"/>
													<input type="submit" value="留言資料庫" formaction="<c:url value='/messageDB_CMS' />"/>
													<input type="button" value="刪除" id="delete" />
<%-- 													<input type="submit" value="刪除" formaction="<c:url value='/deleteArticle_CMS' />"/> --%>
													<input type="submit" value="修改" formaction="<c:url value='/getOldArticle_CMS' />"/>
												</form>
											</td>
						...
						<a href=<c:url value='/createArticle_CMS'/>>新增文章</a>
	...
	$('#tb').on('click', '#delete', function() {
		...
		if (yes) {
			let articleId = $(this).parent().siblings().eq(0).text();
			$.ajax({
				type : 'post',
				url : '<c:url value='/deleteArticle_CMS'/>',
				data : {
					articleId : articleId,
				},
